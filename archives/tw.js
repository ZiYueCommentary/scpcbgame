...
// 网页简繁体转换
    ...
    function zh_tran(go) {
        if (go) trad_conv = go;
        setCookie('trad_conv', trad_conv, zh_expires);
        if (go == 'n') {
    ...
    function zh_getLang() {
        if (getCookie('trad_conv')) {
            trad_conv = getCookie('trad_conv');
            return true;
        }
        ...
        setCookie('trad_conv', trad_conv, zh_expires);
        if (trad_conv == zh_default) return false;
        return true;
    }
    function zh_init() {
        zh_getLang();
        c = document.getElementById(zh_class + '_' + trad_conv);
        if (trad_conv != zh_default) {
            if (window.onload) {
